...
export default function WishlistPage() {
 ...
 async function handleAddToCart(e: React.MouseEvent, item: WishlistItem) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#F4F6F2] px-4 md:px-10 py-8 md:py-12">
        <p className="text-[#6B7B76] text-sm">Loading your wishlist...</p>
      </div>
    );
  }
  ...
  if (items.length === 0) {
    return (
      <div className="min-h-screen bg-[#F4F6F2] px-4 md:px-10 py-8 md:py-12">
        <h1 className="font-display text-2xl md:text-4xl text-[#1A2E2A] mb-6 md:mb-8">Your Wishlist</h1>
        <p className="text-[#6B7B76] text-sm">You haven't added anything to your wishlist yet.</p>
        <button
          ...
          >
            <div className="relative">
              ...
              <button
                ...
                className="absolute top-2 md:top-3 right-2 md:right-3 bg-white/90 rounded-full w-7 h-7 md:w-8 md:h-8 flex items-center justify-center text-[#4A6B5A]"
...
<div className="p-3 md:p-4">
              ...
              <div className="flex items-center justify-between">
                <span className="text-[#4A6B5A] font-semibold text-sm md:text-base">Rs {item.price}</span>
                <button
