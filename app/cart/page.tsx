...
export default function CartPage() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#FAF8F5] px-4 md:px-10 py-8 md:py-12">
        <p className="text-[#8A7F76] text-sm">Loading your cart...</p>
      </div>
    );
  }
  ...
  if (items.length === 0) {
    return (
      <div className="min-h-screen bg-[#FAF8F5] px-4 md:px-10 py-8 md:py-12">
        <h1 className="font-display text-3xl md:text-4xl text-[#2B2420] mb-8">Your Cart</h1>
        <p className="text-[#8A7F76] text-sm">Your cart is empty.</p>
        <button
  ...
  return (
    <div className="min-h-screen bg-[#FAF8F5] px-4 md:px-10 py-8 md:py-12">
      <h1 className="font-display text-3xl md:text-4xl text-[#2B2420] mb-6 md:mb-8">Your Cart</h1>
          ...
          >
            ...
            <div className="flex-1 hidden sm:block">
              ...
              <div className="flex items-center gap-2 mt-2">
                <button
                  ...
                  className="w-7 h-7 border border-[#E7DDD4] rounded text-sm flex-shrink-0"
                >
                  -
                </button>
            ...
            <div className="flex items-center justify-between sm:hidden">
              <div className="flex items-center gap-2">
                <button
                  onClick={() => handleUpdateQty(item.id, item.qty - 1)}
                  className="w-7 h-7 border border-[#E7DDD4] rounded text-sm"
                >
                  -
                </button>
                <span className="text-sm w-6 text-center">{item.qty}</span>
              ...
              <button
                ...
                className="text-xs text-[#8A7F76] hover:text-[#8C4A3A]"
              >
                Remove
              </button>
            </div>
            ...
            <button
              onClick={() => handleRemove(item.id)}
              className="hidden sm:block text-xs text-[#8A7F76] hover:text-[#8C4A3A]"
            >
              Remove
            </button>
          </div>
      ...
      <div className="max-w-2xl mt-6 bg-white/60 rounded-xl p-6 flex flex-col sm:flex-row gap-4 sm:justify-between sm:items-center">
        <span className="font-display text-lg text-[#2B2420]">
          Total: <span className="text-[#8C4A3A]">Rs {total}</span>
        </span>
        <button
          onClick={handleCheckout}
          className="w-full sm:w-auto bg-[#8C4A3A] hover:bg-[#7a3f31] text-white font-medium py-3 px-8 rounded-lg transition-colors text-sm tracking-wide"
        >
          Proceed to Checkout
        </button>
      </div>
